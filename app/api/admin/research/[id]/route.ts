...
export async function GET(
  ...
  const item = await Research.findById(id).lean();
  if (!item) {
...
export async function PATCH(
  ...
  await dbConnect();
  const { id } = await params;
  const existing: any = await Research.findById(id);
  if (!existing) {
    return NextResponse.json(
      { ok: false, error: "not-found" },
      { status: 404 },
  ...
  if (changes.slug && changes.slug !== existing.slug) {
    const clash = await Research.findOne({
      slug: changes.slug,
      _id: { $ne: existing._id },
    }).lean();
    if (clash) {
